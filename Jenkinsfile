pipeline {
    ...
    stages {
        ...
        stage('build') {
            when { branch 'master' }
            steps {
                sh "ssh ubuntu@10.20.1.136 'cd ~/'shopperzoid_v1.0.1' ; mvn clean package -DskipTests'"
                sh "ssh ubuntu@10.20.1.136 'cd ~/'shopperzoid_v1.0.1'/frontend ; npm install'"
                sh "ssh ubuntu@10.20.1.136 'cd ~/'shopperzoid_v1.0.1'/frontend ; ng build'"
